...
class LangZH(output: PackOutput) : LanguageProvider(output, ExtendedAEPlusClient.MODID, "zh_cn") {
    override fun addTranslations() {
        ...
        UtilKeyBuilder.ofDataGen(UtilKeyBuilder.message).template(UtilKeyBuilder.actionBar)
        ...
        UtilKeyBuilder.ofDataGen(UtilKeyBuilder.screenTooltip)
            ...
            .branch("none", "禁用")
            .branch("when_open", "打开时")
            .branch("auto_open", "自动打开")
            .branch("server_by_group", "服务端按组")
            .branch("eaep_by_name", "EAEP按名称")
            .buildInto("修改上传模式")
        ...
        UtilKeyBuilder.ofDataGen(UtilKeyBuilder.message)
            ...
            .buildInto("[EAEP] 样板上传超时, 请检查网络或重试")
        UtilKeyBuilder.ofDataGen(UtilKeyBuilder.message)
            .addStr("provider_list")
            .addStr("group_not_found")
            .buildInto("[EAEP] 选中的供应器在服务端不可见, 已取消上传")
