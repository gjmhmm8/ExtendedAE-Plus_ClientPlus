...
class LangEN(output: PackOutput) : LanguageProvider(output, ExtendedAEPlusClient.MODID, "en_us") {
    override fun addTranslations() {
        ...
        UtilKeyBuilder.ofDataGen(UtilKeyBuilder.screen)
            ...
            .branch(
                "description",
                "Tasks that crafts this item will be completed automatically after all the patterns have been pushed and the terminal is opened"
            )
            .branch(
                "only_completion",
                "This pattern only accepts automatic completion or manual cancellation, and cannot be detected to return to completion"
            )
        ...
        UtilKeyBuilder.ofDataGen(UtilKeyBuilder.config)
            ...
            .branch(
                "tooltip",
                "Quartz Cutting Knife automatically makes quantities of each Inscriber Name Press for processing patterns with custom-named Items"
            )
